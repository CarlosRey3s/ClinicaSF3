...
namespace General.CLS
{
    public class Factura
    {
        ...
        {
            Decimal resultado = -1; // Valor por defecto en caso de error
            DataLayer.DBOperaciones operacion = new DataLayer.DBOperaciones();

            try
            {
                // Inicializa el parámetro de salida
                operacion.Consultar("SET @TotalMedicamentos = 0;");

                // Llama al procedimiento almacenado
                operacion.Consultar($"CALL ObtenerTotalMedicamentosPorFactura({ID}, @TotalMedicamentos);");

                // Recupera el valor del parámetro de salida
                DataTable dt = operacion.Consultar("SELECT @TotalMedicamentos AS TotalMedicamentos;");

                if (dt.Rows.Count > 0)
                {
                    resultado = Convert.ToDecimal(dt.Rows[0]["TotalMedicamentos"]);
                }
            }
            catch (Exception ex)
            {
